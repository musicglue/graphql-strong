...
test("will execute the onExecute function if provided for queries", async () => {
  ...
  expect(await graphql(schema, "{ a b c }", source1, context1)).toEqual({
    data: { a: 1, b: 2, c: 3 },
  });
  expect(await graphql(schema, "{ a b c }", source2, context2)).toEqual({
    data: { a: 1, b: 2, c: 3 },
  });
...
test("will execute the onExecute function if provided for mutations", async () => {
  ...
  const context1 = Symbol("context1");
  const context2 = Symbol("context2");
  expect(await graphql(schema, "mutation { a b c }", source1, context1)).toEqual({
    data: { a: 1, b: 2, c: 3 },
  });
  expect(await graphql(schema, "mutation { a b c }", source2, context2)).toEqual({
    data: { a: 1, b: 2, c: 3 },
  });
  expect(onExecute.mock.calls.length).toBe(2);
  expect(onExecute.mock.calls[0].length).toBe(3);
